...
my ($caHitBsec, $preCompSec, $postCompSec)                              = (0, 0, 0);
...
while ($elapsed <= $testLen) {
  ...
  if ($elapsed > 0) {
    ...
    $cBytesIn   = sprintf("%.0f", ($dataVals->{$dataOids{sysStatClientBytesIn}}  - $oldData{cBytesIn}) / $loopTime);
    $cBytesOut  = sprintf("%.0f", ($dataVals->{$dataOids{sysStatClientBytesOut}} - $oldData{cBytesOut}) / $loopTime);
    $sBytesIn   = sprintf("%.0f", ($dataVals->{$dataOids{sysStatServerBytesIn}}  - $oldData{sBytesIn}) / $loopTime);
    $sBytesOut  = sprintf("%.0f", ($dataVals->{$dataOids{sysStatServerBytesOut}} - $oldData{sBytesOut}) / $loopTime);
    $cNewConns  = sprintf("%.0f", ($dataVals->{$dataOids{sysStatClientTotConns}} - $oldData{cTotConns}) / $loopTime);
    ...
    $cacheMiss  = sprintf("%.0f", ($dataVals->{$dataOids{waCacheMiss}}           - $oldData{waCacheMiss}) / $loopTime);
    $cacheHitBytes = sprintf("%.0f", ($dataVals->{$dataOids{waCacheHitBytes}}    - $oldData{waCacheHitBytes}) / $loopTime);

    # Correct the scale on certain data points
    $cBitsIn     = sprintf("%.0f", (($cBytesIn * 8)  / 1000000));
    $cBitsOut    = sprintf("%.0f", (($cBytesOut * 8) / 1000000));
    $preCompSec  = sprintf("%.0f", ($waPreComp       / 1000000));
...
write;
  ...
  $oldData{waPostComp}      = $dataVals->{$dataOids{httpPostCompressBytes}};
  $oldData{waCacheHits}     = $dataVals->{$dataOids{waCacheHits}};
  $oldData{waCacheMiss}     = $dataVals->{$dataOids{waCacheMiss}};
  $oldData{waCacheHitBytes} = $dataVals->{$dataOids{waCacheHitBytes}};
...
sub get_f5_oids() {
  my %oidlist = (
      ...
      'waCacheMiss'             => '.1.3.6.1.4.1.3375.2.1.1.2.23.3.0',
      'waCacheMissAll'          => '.1.3.6.1.4.1.3375.2.1.1.2.23.4.0',
